...
private:
  ...
  const std::vector<HTMLToken<CharT>> _Tokenize(const CharT *const _buffer, const size_t _bufferLen, const std::vector<std::string> &_voidElements) const {
    States state;
...
public:
  ...
  void Parse(const CharT *const _buffer, const size_t _bufferLen, const std::vector<std::string> &_voidElements = GParsing::HTML_VOID_ELEMENTS) {
    std::vector<HTMLToken<CharT>> tokens;
    tokens = _Tokenize(_buffer, _bufferLen, _voidElements);
